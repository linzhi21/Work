        ...
        <el-row>
          <el-col :span="item.span" v-for="(item,index) in formList" :key="index">
            <el-form-item :prop="item.props" :label="item.label" :label-width="item.labelWidth" style="width: 100%;">
              ...
              <el-time-picker v-model="form[item.props]" placeholder="任意时间点" v-else-if="item.type==='timePicker'"></el-time-picker>
      ...
      ok(dialogType) {
        ...
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            ...
            let startTimeM=new Date(this.form.startTime).getMinutes()<10?'0'+new Date(this.form.startTime).getMinutes():new Date(this.form.startTime).getMinutes();
            let startTimes=new Date(this.form.startTime).getSeconds()<10?'0'+new Date(this.form.startTime).getSeconds():new Date(this.form.startTime).getSeconds();
            let endTimeH=new Date(this.form.endTime).getHours()<10?'0'+new Date(this.form.endTime).getHours():new Date(this.form.endTime).getHours();
            let endTimeM=new Date(this.form.endTime).getMinutes()<10?'0'+new Date(this.form.endTime).getMinutes():new Date(this.form.endTime).getMinutes();
