  ...
  <div>
    ...
    <el-dialog :title="dialog.title" :visible.sync="dialog.isShow" width="80%">
        ...
        <el-row style="background: #f5f5f5;padding: 0.2rem">
          ...
          <el-col :span="12">
            <el-form-item label="报修区域" prop="areaId">
              <el-select v-model="dialog.data.areaInfo" value-key="id" placeholder="请选择报修区域"
                @change="({id,label})=>(dialog.data.areaId=id, dialog.data.areaName=label)">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="工段" prop="workshopSectionId">
              <el-select v-model="dialog.data.workshopSectionInfo" value-key="id" placeholder="请选择工段"
                @change="({id,label})=>(dialog.data.workshopSectionId=id, dialog.data.workshopSectionName=label)">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="班次" prop="workshopShiftId">
              <el-select v-model="dialog.data.workshopShiftInfo" value-key="id" placeholder="请选择班次"
                @change="({id,label})=>(dialog.data.workshopShiftId=id, dialog.data.workshopShiftName=label)">
                <el-option
                  v-for="item in dialog.list.workshopShiftList"
                  :key="item.id"
                  :label="item.label"
                  :value="item"
                ></el-option>
          ...
          <el-col :span="12">
            <el-form-item label="故障种类" prop="breakdownType">
              <el-select v-model="dialog.data.breakdownTypeInfo" value-key="id" placeholder="请选择故障种类"
                @change="({id,label})=>(dialog.data.breakdownType=id, dialog.data.breakdownTypeName=label)">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="紧急程度" prop="emergencyDegree">
              <el-select v-model="dialog.data.emergencyDegreeInfo" value-key="id" placeholder="请选择紧急程度"
                @change="({id,label})=>(dialog.data.emergencyDegree=id, dialog.data.emergencyDegreeName=label)">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="报修内容" prop="content">
              <el-input v-model="dialog.data.content"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="24">
            <el-form-item label="故障描述" prop="description">
              <el-input type="textarea" :rows="4" v-model="dialog.data.description"></el-input>
            </el-form-item>
          </el-col>
      ...
      <el-row type="flex" justify="center">
        ...
        <el-button v-if="!dialog.disabled" type="primary" @click="$refs.dialog.validate(validate=>validate&&save())">保存</el-button>
      </el-row>
    </el-dialog>
  </div>
</template>
...
import { factoryManage, workshopManage, workshopAreaManage, workshopSectionManage, workshopShiftManage } from '../../lib/api/workshopSettingsManage';
...
export default {
  ...
  methods: {
    ...
    /** 点击新增按钮 */
    showAddModal() {
      const { id, name, factoryId, workshopId } = this.$store.state.user.userInfo.principal;
      this.dialogTitle = "新增";
    ...
    showDetailModal(row) {
      ...
      showDetail(null, row.id).then((res) => {
        ...
        data.workshopSectionInfo = {id:data.workshopSectionId, label:data.workshopSectionName};
        
        data.workshopShiftInfo = {id:data.workshopShiftId, label:data.workshopShiftName};

        data.breakdownTypeInfo = {id:data.breakdownType, label:data.breakdownTypeName};
