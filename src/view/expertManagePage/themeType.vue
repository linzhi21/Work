  ...
  <div>
    ...
    <el-row>
      ...
      <tpms-table
        ...
        :total="table.total"
        :columns="[
            { label: '主题类型名称', props: 'name' },
            { label: '设备资料类型描述', props: 'description' },
            { label: '是否启用', props: 'enable', translate: (val) => val?'已启用':'已禁用' },
        ]"
        @inquireTableData="inquireTableData"
    ...
    >
      ...
      >
        <el-form-item label="主题帖类型名字" prop="name" required>
          <el-input v-model="dialog_add.data.name"></el-input>
        </el-form-item>
        <el-form-item label="设备资料类型描述" prop="description" required>
          <el-input v-model="dialog_add.data.description" type="textarea"></el-input>
        </el-form-item>
      ...
      <el-row type="flex" justify="center">
        ...
        <el-button
          type="primary"
          @click="$refs.dialog_add.validate(validate=>validate&&beforeAddThemeType())"
        >保存</el-button>
      </el-row>
    </el-dialog>
    ...
    >
      ...
      >
        ...
        <el-form-item label="设备资料类型描述" prop="description" required>
          <el-input v-model="dialog_edit.data.description" type="textarea"></el-input>
        </el-form-item>
        <el-form-item label="启用">
...
import { themeTypeManage } from '../../lib/api/expert';
export default {
  data() {
    return {
      authFormList: [
        { label: "主题类型名称", props: "name", value: "", 'label-width': '100px' },
      ],
      table: {
        data: [],
        total: 0
  ...
  methods: {
    ...
    getTableData() {
      ...
      themeTypeManage.getList({ ...data, ...pageData }).then(res => {
    ...
    addThemeType() {
      ...
      themeTypeManage.add(data).then(res => {
    ...
    editClick({id}) {
      ...
      themeTypeManage.getDetail(null, id).then(res => {
    ...
    edit() {
      ...
      themeTypeManage.edit(data, id).then(res => {
